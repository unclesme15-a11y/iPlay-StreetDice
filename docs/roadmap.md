# Roadmap

## Phase 1 - Rules Lock

- Confirm final shooter-retains-dice rule after point-phase losses.
- Confirm payout model.
- Confirm side-bet menu.
- Confirm fade timing window.
- Confirm streak meter thresholds.
- Confirm Double Up risk model.

## Phase 2 - Backend Prototype

- Create minimal server-authoritative dice backend.
- Add deterministic test mode for known dice outcomes.
- Add full rule tests.
- Add table voice token gate.
- Add simple local smoke verifier.

## Phase 3 - Unity Greybox

- Build top-down angled dice lane.
- Add 5-seat layout.
- Add dice color selection.
- Add Roll, Fade/Catch, Run Same, Double Up buttons.
- Add magnifier/result HUD.
- Add streak meter and hot dice mode.

## Phase 4 - Real Clip / Kling Lane

- Produce reference-safe clips for:
  - Shooter ready.
  - Come-out roll.
  - Point roll.
  - Catcher fade.
  - Hot dice activation.
- Keep Unity dice/result authoritative over the clip.

## Phase 5 - Multiplayer Feel

- Add voice UI.
- Add side-betting UI.
- Add table talk indicators.
- Add player reactions.
- Add spectator-like camera cuts while preserving gameplay readability.

## Phase 6 - Production Readiness

- Security and abuse controls.
- Mobile performance checks.
- Device testing.
- Commercial asset audit.
- Legal review before any real-money or cash-equivalent direction.
